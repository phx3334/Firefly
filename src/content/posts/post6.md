...
`1`服务端先关的代价与对策：服务端 TIME_WAIT 会占 fd 和内存，靠 `tcp_tw_reuse`、多监听 IP、以及 SO_REUSEADDR（重启时快速重绑端口）缓解。  
`2`最糟的情况是服务端「被动关闭后忘了 close 应用层 socket」——CLOSE_WAIT 大量堆积（典型的 fd 泄漏），这是应用代码 bug，内核参数救不了，只能修代码。  
`3`一句话总结：**资源多、可控的一端主动关闭**，把 TIME_WAIT 转移到能承受它的一端；客户端分散且端口稀缺，所以绝大多数公网服务设计为服务端先关。
