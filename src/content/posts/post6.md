...
#### DHCP
DHCP（Dynamic Host Configuration Protocol，动态主机配置协议）是网络中的一种应用层协议，用来自动给设备分配 IP 地址等网络参数，避免手动配置的麻烦和冲突  
#### NAT(网络地址转换)
**原理**：私有 IP（`10.x`/`172.16~31.x`/`192.168.x`）不能直接上公网，NAT 在出口设备（路由器/网关）上**改写数据包的地址**，让私网设备共用少量公网 IP 上网：
```
手机(192.168.1.2) → 路由器改写源地址为路由器的公网ip 220.181.38.150 → 百度
百度响应 → 路由器查表改回 192.168.1.2 → 手机
```
**NAT 转换表**：路由器靠**端口号**区分不同设备，记住"哪个私网设备对应哪个公网端口"：
```
192.168.1.2:50000  →  220.181.38.150:40001
192.168.1.3:50000  →  220.181.38.150:40002
```
**常见类型**：
| 类型                 | 作用                                                                 | 场景              |
| -------------------- | -------------------------------------------------------------------- | ----------------- |
| SNAT（源地址转换）   | 例如自己私人使用的设备就是通过路由器做的SNAT把内网源IP换成公网ip上网 | 私网设备上网      |
| DNAT（目标地址转换） | 改写目标地址，公网端口映射到内网机器                                 | 端口映射/端口转发 |
**和 SSH 端口转发的联系**：`ssh -R 8080:localhost:80` 本质是在**应用层模拟 DNAT**——把公网服务器的 8080 端口转发到内网机器的 80 端口；路由器 DNAT 在网络层做类似的事，但无需开 SSH。副作用：NAT 下外网无法主动连入内网（需端口映射或内网穿透），这是 IPv6 要解决的根本问题。
#### 交换机和路由器
**原理**：两者都转发数据，但工作层次和转发依据完全不同：
**对比**：
| 对比项   | 交换机             | 路由器             |
| -------- | ------------------ | ------------------ |
| 工作层   | 数据链路层（二层） | 网络层（三层）     |
| 转发依据 | MAC 地址表         | 路由表（IP）       |
| 连接对象 | 同一网段的设备     | 不同的网络/网段    |
| 典型位置 | 局域网内部         | 网络出口（接公网） |
| 能否上网 | 不能，无公网出口   | 能，网关设备       |
**单臂路由（Router-on-a-Stick）**：跨 VLAN 通信要走三层路由，但 VLAN 之间在交换机上二层隔离，路由功能谁来做？**单臂路由**——路由器只用一个物理接口接交换机 Trunk 口，再在该接口上划出多个**子接口**（如 `eth0.10`、`eth0.20`），每个子接口绑定一个 VLAN 并充当该 VLAN 的网关，一台路由器就能转发任意 VLAN 之间的流量。

**举例：路由器如何跨 VLAN 通信**：
```
PC-A（VLAN 10）192.168.10.2 ──→  PC-B（VLAN 20）192.168.20.2
1. PC-A 发现目标不在自己网段 → 把帧发给网关 192.168.10.1（路由器子接口 eth0.10）
2. 交换机 Access 口收到 → 打上 VLAN 10 标签 → 从 Trunk 口发给路由器
3. 路由器 eth0.10 解封装，查路由表 → 从 eth0.20 发出，打上 VLAN 20 标签
4. 交换机 Trunk 口收到 → 去掉标签 → 从 VLAN 20 的 Access 口发给 PC-B
```
整个过程交换机只做二层转发（认 VLAN 标签），路由器只做三层寻路（认 IP），各司其职。

**三层交换机**：普通交换机（二层）加路由能力（三层），局域网内跨 VLAN 通信用 VLANIF 接口走"一次路由、多次交换"，比单臂路由更快，是园区网的标配。
**一句话总结**：交换机在楼内分发（认 MAC），路由器在门口指路（认 IP）；VLAN 让交换机有了"门牌号"，跨门牌号找路就得上路由器。
#### VLAN（虚拟局域网）
**原理**：交换机默认所有端口在同一个广播域——一台设备发广播，所有人都会收到，设备多时网络变慢。VLAN 把一个物理交换机**划分成多个逻辑局域网**，不同 VLAN 的设备二层隔离（收不到彼此的广播和直接通信）：
```
交换机（物理上 1 台，逻辑上分 3 个）
VLAN 10：端口 1-8   →  市场部
VLAN 20：端口 9-16  →  技术部
VLAN 30：端口 17-24 →  财务部
技术部发广播 → 只有 VLAN 20 内的端口收到，市场部/财务部无感
```
**关键点**：
| 概念         | 说明                                                      |
| ------------ | --------------------------------------------------------- |
| 广播域隔离   | VLAN 越大广播域越小，避免"广播风暴"                       |
| VLAN ID      | 1~4094 可用，划分依据                                     |
| 802.1Q 标签  | 在以太网帧里打 4 字节标签，标明属于哪个 VLAN              |
| Access 端口  | 接终端设备（PC），只属于 1 个 VLAN，出厂不打标            |
| Trunk 端口   | 交换机之间级联，允许带标签的多个 VLAN 通过                |
| 跨 VLAN 通信 | 二层不通，必须走三层：路由器单臂路由 / 交换机 VLANIF 接口 |
...
### time_wait时间为什么是2MSL
`MSL`：报文最大生存时间，即一个报文在网络中存在的最长时间，Linux 实测 MSL=30s，2MSL=60s。  
`1`保证最后的 ACK 能重传：若主动关闭方发的 ACK 丢失，被动关闭方（LAST_ACK）会重发 FIN，主动关闭方必须留在 TIME_WAIT 才能重回应答，否则回 RST 导致对端异常关闭。  
`2`让旧报文自然消亡：防止延迟到达的旧报文被「复用了相同四元组的新连接」误收，污染新连接数据流。等满 2MSL，旧连接的所有报文都已超出生存时间被丢弃，复用四元组才安全。

### RST什么情况下发送
RST（复位）是 TCP 的「立即中止」信号，不带确认、不进缓冲区、不等重传，收到后连接直接拆除，上层应用通常读到 `Connection reset by peer`（ECONNRESET）错误。SRE 排查时常见这几类：
`1`访问了未监听的端口：SYN 到达但目标端口没有进程在 listen，内核直接回 RST（对 UDP 则是 ICMP port unreachable）。这也是端口扫描判断「端口关闭」的原理。  
`2`连接不存在或已废弃：报文的四元组在内核连接表里查不到（对端机器重启、连接早已超时回收），内核回 RST 告知异常，而不是默默丢弃。  
`3`半开连接检测：一端崩溃重启后收到旧连接的数据，因内核已无此连接状态，回 RST 让对端尽快感知并释放资源。  

### 高并发场景下，减少time_wait累计的主要方法
TIME_WAIT 只出现在主动关闭方，治理思路：从源头少建连 > 复用状态 > 扩大四元组空间。  
`1`长连接/连接池：复用连接减少建连和挥手次数，TIME_WAIT 根本不产生，是治本方案。  
`2`tcp_tw_reuse=1：允许客户端复用 TIME_WAIT 的四元组发新连接，需配合 tcp_timestamps（默认开启）判断旧报文不串扰，仅对发起连接一方生效。  
`3`扩大端口范围：`net.ipv4.ip_local_port_range="1024 65535"`，TIME_WAIT 占的是四元组，源端口是唯一自由变量，端口变多则四元组组合翻倍，推迟耗尽时间。  
`4`多目标 IP/端口：服务端多实例、DNS 多 A 记录，目的地址也可变，连接容量按倍数增长。  
`5`不推荐 tcp_tw_recycle：NAT 下多设备共享出口 IP 导致时间戳乱序、误丢 SYN，Linux 4.12 已移除。

### 服务端和客户端谁先发起关闭连接的业务场景
四次挥手后**主动关闭方进入 TIME_WAIT**，所以「谁先关」本质是在决定「谁承担 TIME_WAIT 的资源代价」，工程上要刻意设计：
| 场景                           | 建议谁先关               | 原因                                                                                                 |
| ------------------------------ | ------------------------ | ---------------------------------------------------------------------------------------------------- |
| HTTP 服务（短连接）            | 服务端先关               | 客户端海量且分散（NAT 后共享端口稀缺），服务端资源可控、可扩容，TIME_WAIT 堆在服务端比堆在客户端安全 |
| HTTP 服务（keep-alive 长连接） | 服务端先关（超时回收）   | 空闲连接超时后由服务端统一回收，避免客户端遗忘关闭导致服务端 CLOSE_WAIT 堆积                         |
| 反向代理/网关 → 上游           | 网关侧控制               | 网关与上游都在自己可控范围内，用连接池长连接，减少挥手次数                                           |
| 数据库/缓存客户端              | 客户端先关（用完归还池） | 服务端连接数是宝贵资源，由客户端连接池管理生命周期；服务端 `wait_timeout` 只作兜底回收               |
| 客户端未知行为的服务           | 服务端设 idle 超时       | 客户端可能异常退出不挥手，服务端靠空闲超时主动关，避免孤儿连接                                       |
`1`服务端先关的代价与对策：TIME_WAIT 只占内核内存和 conntrack 表项，**不占 fd**（`close()` 后 fd 已释放，占 fd 的是 CLOSE_WAIT 泄漏）；真正扎手的坑是**重启时 `bind` 报 Address already in use**——TIME_WAIT 的 socket 也算端口占用，需在 bind 前设 `SO_REUSEADDR` 放行（只对 bind 生效）。  
`2`最糟的情况是服务端「被动关闭后忘了 close 应用层 socket」——CLOSE_WAIT 大量堆积（典型的 fd 泄漏），这是应用代码 bug，内核参数救不了，只能修代码。  
`3`一句话总结：**资源多、可控的一端主动关闭**，把 TIME_WAIT 转移到能承受它的一端。单台用户设备的连接虽然很少，但公网客户端藏在运营商 NAT 后——成千上万用户共享同一个出口 IP 的临时端口池，客户端侧堆 TIME_WAIT 烧的是这个共享池；且亿万客户端设备不可控、不可修，服务端则可控可调优。所以绝大多数公网服务设计为服务端先关。


### 为什么大部分高并发场景一般采用长连接
`1`短连接无法复用拥塞控制状态（拥塞窗口、慢启动阈值、RTT 估算等），每次传输都要重新慢启动，拥塞窗口还没爬上来连接就关了，高频小请求吞吐低。  
...
## 补充
**常见错误报警数字**：  403（权限问题），404（资源不存在），504（网关超时，上游服务慢），503（服务不可用）
**常见默认端口（一般不能改的）**： 80：HTTP —— 浏览器访问 http://域名 默认走 80，改成别的用户访问会失败  
443：HTTPS —— 同上，https://域名 默认走 443  
25：SMTP —— 邮件发送协议固定端口   
