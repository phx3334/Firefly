...
和 VXLAN 思路完全一样（跨三层用隧道兜底），只是"壳"更薄：VXLAN 是把二层帧套进 UDP，IPIP 直接把整个 Pod IP 包再套一层 IP 头（开销约 20 字节，比 VXLAN 的 50 字节小）。calico 里这个隧道设备叫 `tunl0`，外层目的 IP 填对端节点 IP，纯三层转发、没有 VNI 隔离。一句话：**要大二层/多租户用 VXLAN，只要三层互通、要轻量就 IPIP 更划算。**
